...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), "New address should not be null");
        _owner = newOwner;
        emit OwnershipTransferred(_owner);
    }
}
...
contract Pausable is Ownable {
    ...
    function setIsPause(bool status) public onlyOwner {
        require(
            status != _paused,
            "Paused status is already in the expected state"
        );
        _paused = status;
        if (status) {
